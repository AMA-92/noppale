...
export default function Sales() {  
  ...
  // ├ëcouter les changements en temps r├®el sur les ventes
  useSalesRealtime(() => {
    loadSales()
  })
  ...
  /** Compl├¿te les prix manquants depuis le catalogue produits */
  ...
  const selectProduct = (product) => {
    try {
      ...
      if (unitPrice <= 0) {
        toast.error('Prix du produit invalide ÔÇö d├®finissez un prix de vente dans Produits')
        return
      }
      ...
      const newItem = {
        ...
        productUnit: product.barcode || 'unit├®'
      }
        ...
        updateCart((currentItems) => {
          ...
          if (existingItem) {
            ...
            if (currentQty >= availableStock) {
              toast.error(`Stock insuffisant ! Il ne reste que ${availableStock} unit├®(s) de ${product.name}`)
              return currentItems
            }
      ...
      setProductSearch('')
      setCurrentItem(emptyItem)
      toast.success(`${product.name || 'Produit'} ajout├® au panier`)
    } catch (error) {
  ...
  const updateItemQuantity = (productId, quantity) => {
    // R├®cup├®rer le produit pour v├®rifier le stock disponible
    const product = products.find(p => p.id === productId)
    ...
    if (newQuantity > availableStock) {
      toast.error(`Quantit├® trop ├®lev├®e ! Il ne reste que ${availableStock} unit├®(s) de ${product.name}`)
      return
    }
  ...
  const handleSave = async (e) => {
    ...
    if (invalidItem) {
      toast.error(`Prix manquant pour ┬½ ${invalidItem.productName} ┬╗. D├®finissez un prix de vente dans Produits.`)
      return
    }
    ...
    if (sanitizedSale.paymentMethod === 'especes') {
      ...
      if (received > 0 && received < saleTotal) {
        toast.error('Le montant re├ºu est insuffisant')
        return
      }
    }
    ...
    setSaving(true)
    try {
      // Enregistrer la vente avec Supabase (g├¿re automatiquement le stock)
      ...
      toast.success('Vente enregistr├®e et stock mis ├á jour')
      cartItemsRef.current = []
      setCartItems([])
      setShowModal(false)
      setForm(emptySale)
  ...
  const saveEditedSale = () => {
    // Pour l'instant, la modification de vente est d├®sactiv├®e avec Supabase
    // car elle n├®cessite une logique complexe de gestion du stock
    toast.error('La modification de vente n\'est pas encore support├®e avec Supabase')
  ...
  const printAdvancedFacture = () => {
    ...
    import('jspdf').then(({ jsPDF }) => {
      import('html2canvas').then(({ default: html2canvas }) => {
        ...
        // Cr├®er le nom du fichier avec nom client et date
        ...
        tempDiv.innerHTML = `
          ...
          <div style="margin: 20px 0;">
            ...
            ${selectedSale.amountReceived > 0 ? `
              <p><strong>Montant re├ºu:</strong> ${formatCurrency(selectedSale.amountReceived)}</p>
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            ...
            <tbody>
              ...
              `).join('') : '<tr><td colspan="4" style="border: 1px solid #ddd; padding: 8px; text-align: center;">Aucun article d├®taill├®</td></tr>'}
            </tbody>
            <tfoot>
          ...
          <div style="text-align: center; margin-top: 30px; font-size: 12px; color: #666;">
            ...
            ${(shopInfo.address || shopInfo.phone || shopInfo.email) ? `
              <p>
                ${shopInfo.address || ''}${shopInfo.address && (shopInfo.phone || shopInfo.email) ? ' ÔÇó ' : ''}
                ${shopInfo.phone ? '­ƒô× ' + shopInfo.phone : ''}${shopInfo.phone && shopInfo.email ? ' ÔÇó ' : ''}
                ${shopInfo.email ? 'Ô£ë´©Å ' + shopInfo.email : ''}
              </p>
            ` : ''}
          </div>
        `
        ...
        html2canvas(tempDiv, {
          ...
          toast.success('Facture PDF g├®n├®r├®e avec succ├¿s')
        }).catch(error => {
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-slate-600 mb-2">
            {search ? 'Aucune vente trouv├®e' : 'Aucune vente'}
          </h3>
          <p className="text-slate-500">
            {search ? 'Essayez une autre recherche' : 'Enregistrez votre premi├¿re vente pour commencer'}
          </p>
        </div>
      )}
          ...
          <div className="modal-content sale-modal-content max-w-2xl">
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 min-h-0">
                      ...
                      <div key={item.productId} className="flex items-center justify-between gap-2 bg-white rounded-lg p-2 border border-primary-100">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <input
                            ...
                            aria-label={`Quantit├® ${item.productName}`}
                          />
                          <button
                            type="button"
              ...
              <div className="border-t pt-4">
                ...
                {/* Montant re├ºu et monnaie */}
                {form.paymentMethod === 'especes' && (
                  <div className="space-y-3">
                    <div>
                      <label htmlFor="sale-amount-received" className="label-field">Montant re├ºu</label>
                      <input
                        ...
                        placeholder="Entrez le montant re├ºu"
                        min="0"
                        step="100"
                      />
                    </div>
                    ...
                    {form.amountReceived > 0 && (
                      <div className={`p-4 rounded-lg border ${
                        ...
                        <div className="flex justify-between items-center">
                          <span className="font-semibold text-slate-800">Monnaie ├á rendre:</span>
                        ...
                        {form.change >= 0 ? (
                          <p className="text-sm text-green-700 mt-1">Ô£à Montant suffisant</p>
                        ) : (
                          <p className="text-sm text-red-700 mt-1">ÔØî Montant insuffisant</p>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="credit">├Ç Cr├®dit</option>
                </select>
              </div>
