# Instructions de Déploiement - Étape 4
...
### Étape 2 : Mettre à jour la fonction assistant

1. **Cliquez sur la fonction** `assistant` existante
2. **Cliquez sur "Edit"** ou "Modifier"
3. **Supprimez tout le code existant** dans l'éditeur
4. **Copiez le code** ci-dessous et collez-le :
...
5. **Cliquez sur "Deploy"** ou "Déployer"

### Étape 3 : Vérifier les secrets (si nécessaire)

1. **Allez dans** Project Settings → Edge Functions → Secrets
2. **Vérifiez que les secrets sont configurés** :
   - `ANTHROPIC_API_KEY` = votre clé (commence par `sk-ant-...`)
   - `ANTHROPIC_MODEL` = `claude-haiku-4-5`

### Étape 4 : Vérifier le déploiement

1. **Cliquez sur "Logs"** pour voir les logs de la fonction
2. **Vérifiez qu'il n'y a pas d'erreurs** de déploiement
...
3. Testez : "Bonjour"
4. Testez un parcours complet : "Je veux vendre un produit"

## ✅ Check-list de déploiement

- [ ] Accès au dashboard Supabase
- [ ] Fonction assistant mise à jour avec le nouveau code
- [ ] Déploiement réussi
- [ ] Secrets vérifiés
- [ ] Logs sans erreur
...
- [ ] Test "Bonjour" réussi
- [ ] Test parcours vente réussi

## 🎯 Test rapide après déploiement

Une fois déployé, testez rapidement :

1. **Bonjour** → Devrait répondre par une salutation courte
2. **"Je veux vendre un produit"** → Devrait lancer le parcours vente
3. **"Riz"** → Devrait demander la quantité
4. **"3"** → Devrait demander le paiement
5. **"Espèces"** → Devrait demander le client
6. **"Client test"** → Devrait faire un résumé et demander confirmation
7. **"Non"** → Devrait annuler l'opération

Si tout fonctionne, l'étape 4 est déployée avec succès ! 🚀
