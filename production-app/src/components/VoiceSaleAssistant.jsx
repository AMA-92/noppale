...
function VoiceSaleAssistant({ products: suppliedProducts = [], sales: suppliedSales = [], expenses: suppliedExpenses = [], formatCurrency, onApply, onConfirm, onProductCreate }) {
  ...
  const speak = async (text, listenAfter = false) => {
    if (!text) return
    console.log('🔊 TTS: Tentative de parler:', text)
    const session = speechSessionRef.current
    window.speechSynthesis?.cancel(); audioRef.current?.pause()
    
    // Utiliser directement la synthèse vocale du navigateur (plus fiable sur mobile)
    if (!('speechSynthesis' in window)) {
      console.error('❌ Synthèse vocale non supportée')
      if (listenAfter) { keepListeningRef.current = true; window.setTimeout(() => { if (speechSessionRef.current === session) listenOnce() }, 200) }
      return
    }
    
    if (speechSessionRef.current !== session) {
      console.warn('⚠️ Session expirée, annulation TTS')
      return
    }
    ...
    try {
      ...
      // Charger les voix de manière asynchrone pour mobile
      const loadVoices = () => {
        const voices = window.speechSynthesis.getVoices()
        const languagePrefix = languageRef.current.toLowerCase().split('-')[0]
        utterance.voice = voices.find((voice) => voice.lang?.toLowerCase().startsWith(languagePrefix)) || 
                         voices.find((voice) => voice.lang?.toLowerCase().startsWith('fr')) || 
                         voices[0] || null
        console.log('🎤 Voix sélectionnée:', utterance.voice?.name || 'par défaut')
      }
      
      // Sur certains navigateurs, getVoices est asynchrone
      ...
      utterance.onstart = () => console.log('🔊 TTS: Démarrage de la synthèse')
      utterance.onend = () => {
        console.log('🔊 TTS: Fin de la synthèse')
        if (keepListeningRef.current) {
          console.log('🎤 Relance de l\'écoute après TTS')
          window.setTimeout(() => { if (speechSessionRef.current === session) listenOnce() }, 200)
        }
      }
      ...
      console.log('✅ TTS: Commande speak envoyée')
  ...
  function listenOnce() {
    console.log('🎤 listenOnce appelé')
    if (!SpeechRecognition) { 
      console.error('❌ SpeechRecognition non supporté')
      setError('La reconnaissance vocale nécessite Chrome ou Edge.'); 
      return 
    }
    if (!keepListeningRef.current) {
      console.log('⏸️ Écoute désactivée (keepListeningRef = false)')
      return
    }
    ...
    recognition.continuous = false; // Meilleure compatibilité mobile
    ...
    recognition.onresult = (event) => {
      ...
      console.log('🎤 Texte reconnu:', answer)
    ...
    recognition.onend = () => { 
      ...
      if (keepListeningRef.current && !processingRef.current) { 
        console.log('🎤 Relance automatique de l\'écoute')
        window.clearTimeout(restartTimerRef.current); 
        restartTimerRef.current = window.setTimeout(() => listenOnce(), 500) // Délai augmenté
      } 
    }
    ...
    try { 
      recognition.start()
      console.log('✅ Reconnaissance lancée')
    } catch (error) { 
      console.error('❌ Erreur démarrage reconnaissance:', error)
      recognitionRef.current = null; 
      if (keepListeningRef.current) restartTimerRef.current = window.setTimeout(() => listenOnce(), 1000) // Retry plus long
    }
  }
  ...
  const startFlow = (flow) => {
    ...
    const questions = {
      'sale': languageRef.current === 'en-US' ? 'Great. Which product would you like to sell?' 
            : languageRef.current === 'wo-SN' ? 'Ndax tey. Lu noppal bi ngay yobbe ?'
            : languageRef.current === 'ar-SA' ? 'ممتاز. ما هو المنتج الذي تريد بيعه؟'
            : 'Très bien. Quel produit voulez-vous vendre ?',
      'expense': languageRef.current === 'en-US' ? 'OK. What is the expense description?'
                : languageRef.current === 'wo-SN' ? 'Dama dow. Loolu benn xaalis bi ?'
                : languageRef.current === 'ar-SA' ? 'حسنًا. ما هو وصف المصروف؟'
                : 'D\'accord. Quelle est la description de la dépense ?',
      'addProduct': languageRef.current === 'en-US' ? 'What is the name of the new product?'
                  : languageRef.current === 'wo-SN' ? 'Sama tur bi tudd na ?'
                  : languageRef.current === 'ar-SA' ? 'ما هو اسم المنتج الجديد؟'
                  : 'Quel est le nom du nouveau produit ?',
      'updateProduct': languageRef.current === 'en-US' ? 'Which product would you like to update?'
                    : languageRef.current === 'wo-SN' ? 'Fan noppal bi ngay soppi ?'
                    : languageRef.current === 'ar-SA' ? 'ما هو المنتج الذي تريد تحديثه؟'
                    : 'Quel produit voulez-vous mettre à jour ?'
    }
    return sayAndListen(questions[flow] || 'Que souhaitez-vous faire ?', flow === 'sale' ? 'saleProduct' : flow === 'expense' ? 'expenseDescription' : flow === 'addProduct' ? 'productName' : 'updateProductName')
  }

  const confirmAction = (message, data) => {
    ...
    const fullMessage = `${message} ${confirmQuestion}`
    stateRef.current = { ...stateRef.current, step: 'confirm', data }
    setPhase('confirm')
    setStatus(fullMessage)
    speak(fullMessage, true)
  }

  const execute = async () => {
    const { flow, data } = stateRef.current
    const successMessages = {
      'expense': languageRef.current === 'en-US' ? 'Expense recorded successfully.'
                : languageRef.current === 'wo-SN' ? 'Xaalis bi bind na rek.'
                : languageRef.current === 'ar-SA' ? 'تم تسجيل المصروف بنجاح.'
                : 'La dépense a été enregistrée avec succès.',
      'addProduct': languageRef.current === 'en-US' ? `Product ${data.name} added successfully.`
                  : languageRef.current === 'wo-SN' ? `Noppal ${data.name} togge na rek.`
                  : languageRef.current === 'ar-SA' ? `تمت إضافة المنتج ${data.name} بنجاح.`
                  : `Le produit ${data.name} a été ajouté avec succès.`,
      'updateProduct': languageRef.current === 'en-US' ? `Product ${data.productName} updated successfully.`
                    : languageRef.current === 'wo-SN' ? `Noppal ${data.productName} soppe na rek.`
                    : languageRef.current === 'ar-SA' ? `تم تحديث المنتج ${data.productName} بنجاح.`
                    : `Le produit ${data.productName} a été mis à jour.`,
      'sale': languageRef.current === 'en-US' ? `Sale of ${data.quantity} ${data.product} recorded successfully.`
             : languageRef.current === 'wo-SN' ? `Lik ${data.quantity} ${data.product} bind na rek.`
             : languageRef.current === 'ar-SA' ? `تم تسجيل بيع ${data.quantity} ${data.product} بنجاح.`
             : `La vente de ${data.quantity} ${data.product} a été enregistrée.`
    }
    const errorMessages = {
      'productNotFound': languageRef.current === 'en-US' ? 'I cannot find this product in your catalog.'
                       : languageRef.current === 'wo-SN' ? 'Manuma fekk noppal bi ci sa katalog.'
                       : languageRef.current === 'ar-SA' ? 'لا يمكنني العثور على هذا المنتج في الكتالوج الخاص بك.'
                       : 'Je ne trouve pas ce produit dans votre catalogue.'
    }

    if (flow === 'expense') {
      await appStorage.addExpense({
        description: data.description,
        amount: parseNumber(data.amount),
        category: data.category || 'Générale',
        date: new Date().toISOString().split('T')[0],
        notes: 'Enregistrée par assistant vocal'
      })
      await refreshData()
      setPhase('done')
      setStatus(successMessages['expense'])
      speak(successMessages['expense'])
      return
    }
    if (flow === 'addProduct') {
      const payload = {
        ...
        buyingPrice: parseNumber(data.buyingPrice),
        stock: parseNumber(data.stock),
        minStock: parseNumber(data.minStock),
        category: data.category || '',
        barcode: '',
        description: 'Ajouté par assistant vocal',
        image: ''
      }
      if (onProductCreate) await onProductCreate(payload)
      else await appStorage.addProduct(payload)
      await refreshData()
      setPhase('done')
      setStatus(successMessages['addProduct'])
      speak(successMessages['addProduct'])
      return
    }
    if (flow === 'updateProduct') {
      ...
      const updates = {
        ...
        stock: data.field === 'stock' ? parseNumber(data.value) : product.stock,
        minStock: product.min_stock || 0,
        barcode: product.barcode || '',
        description: product.description || '',
        image: product.image || ''
      }
      if (data.field === 'sellingPrice') updates.sellingPrice = parseNumber(data.value)
      if (data.field === 'buyingPrice') updates.buyingPrice = parseNumber(data.value)
      await appStorage.updateProduct(product.id, updates)
      await refreshData()
      setPhase('done')
      setStatus(successMessages['updateProduct'])
      speak(successMessages['updateProduct'])
      return
    }
    if (flow === 'sale') {
      ...
      const sale = {
        customerName: data.customer || 'Client comptant',
        customerPhone: '',
        items: [{
        ...
        initialPayment: data.paymentMethod === 'credit' ? parseNumber(data.amount) : 0,
        notes: 'Enregistrée par assistant vocal'
      }
      if (onConfirm) await onConfirm(sale)
      else await appStorage.addSale(sale)
      await refreshData()
      setPhase('done')
      const saleSuccessMessage = languageRef.current === 'en-US' ? `Sale of ${quantity} ${product.name} recorded successfully.`
                                : languageRef.current === 'wo-SN' ? `Lik ${quantity} ${product.name} bind na rek.`
                                : languageRef.current === 'ar-SA' ? `تم تسجيل بيع ${quantity} ${product.name} بنجاح.`
                                : `La vente de ${quantity} ${product.name} a été enregistrée.`
      setStatus(saleSuccessMessage)
      speak(saleSuccessMessage)
    }
  }
  ...
  const handleAnswer = async (answer) => {
    ...
    if (step === 'confirm') {
      if (isYes(answer)) {
        ...
        const cancelMessage = languageRef.current === 'en-US' ? 'Operation cancelled.'
                          : languageRef.current === 'wo-SN' ? 'Liggeey bi neex na.'
                          : languageRef.current === 'ar-SA' ? 'تم إلغاء العملية.'
                          : 'Opération annulée.'
        reset()
        setStatus(cancelMessage)
        speak(cancelMessage)
      } else {
        const confirmPrompt = languageRef.current === 'en-US' ? 'Please say yes to confirm or no to cancel.'
                             : languageRef.current === 'wo-SN' ? 'Waxal waaw ngir deggal, walla deedeet ngir neex.'
                             : languageRef.current === 'ar-SA' ? 'يرجى قول نعم للتأكيد أو لا للإلغاء.'
                             : 'Dites oui pour confirmer ou non pour annuler.'
        speak(confirmPrompt, true)
      }
      return
    }
    if (flow === 'sale') {
      if (step === 'saleProduct') {
        const question = languageRef.current === 'en-US' ? 'How many units would you like to sell?'
                      : languageRef.current === 'wo-SN' ? 'Fan yu noppal bi ngay yobbe ?'
                      : languageRef.current === 'ar-SA' ? 'كم وحدة تريد بيعها؟'
                      : 'Combien d\'unités voulez-vous vendre ?'
        return sayAndListen(question, 'saleQuantity', { ...data, product: answer })
      }
      if (step === 'saleQuantity') {
        const question = languageRef.current === 'en-US' ? 'Is the payment in cash, Mobile Money, card or credit?'
                      : languageRef.current === 'wo-SN' ? 'Xoolal bi ñuy waral, lu Mobile Money, walla credit?'
                      : languageRef.current === 'ar-SA' ? 'هل الدفع نقدًا أو عبر المحمول أو بطاقة أو آجل؟'
                      : 'Le paiement est-il en espèces, par Mobile Money, par carte ou à crédit ?'
        return sayAndListen(question, 'salePayment', { ...data, quantity: answer })
      }
      if (step === 'salePayment') {
        const payment = /mobile|wave|orange money|free money/.test(normalize(answer)) ? 'mobile_money' : /carte/.test(normalize(answer)) ? 'carte_bancaire' : /credit|cr[eé]dit/.test(normalize(answer)) ? 'credit' : 'especes'
        const question = languageRef.current === 'en-US' ? 'What is the customer name? Say anonymous if not needed.'
                      : languageRef.current === 'wo-SN' ? 'Sammaaraku bi tudd na? Waxal anonymous su fekke.'
                      : languageRef.current === 'ar-SA' ? 'ما هو اسم العميل؟ قل مجهول إذا لم يكن مطلوبًا.'
                      : 'Quel est le nom du client ? Dites anonyme si nécessaire.'
        return sayAndListen(question, 'saleCustomer', { ...data, paymentMethod: payment })
      }
      if (step === 'saleCustomer') {
        const product = findProduct(data.product)
        if (!product) {
          const error = languageRef.current === 'en-US' ? 'I cannot find this product in your catalog.'
                       : languageRef.current === 'wo-SN' ? 'Manuma fekk noppal bi ci sa katalog.'
                       : languageRef.current === 'ar-SA' ? 'لا يمكنني العثور على هذا المنتج في الكتالوج الخاص بك.'
                       : 'Je ne trouve pas ce produit dans votre catalogue.'
          const question = languageRef.current === 'en-US' ? 'What is the exact product name?'
                        : languageRef.current === 'wo-SN' ? 'Sama tur bi tudd na?'
                        : languageRef.current === 'ar-SA' ? 'ما هو الاسم الدقيق للمنتج؟'
                        : 'Quel est le nom exact du produit ?'
          setError(error)
          return sayAndListen(question, 'saleProduct', data)
        }
        const total = (parseNumber(data.quantity) || 1) * getProductSellingPrice(product)
        const confirmMsg = languageRef.current === 'en-US' ? `I will record a sale of ${data.quantity} ${product.name} for ${spokenMoney(total)}, under the name ${answer}.`
                          : languageRef.current === 'wo-SN' ? `Dama di bind lik ${data.quantity} ${product.name} ci ${spokenMoney(total)}, samaaraku ${answer}.`
                          : languageRef.current === 'ar-SA' ? `سأقوم بتسجيل بيع ${data.quantity} ${product.name} مقابل ${spokenMoney(total)}، باسم ${answer}.`
                          : `Je vais enregistrer une vente de ${data.quantity} ${product.name} pour ${spokenMoney(total)}, au nom de ${answer}.`
        return confirmAction(confirmMsg, { ...data, customer: /anonyme|aucun/.test(normalize(answer)) ? 'Client comptant' : answer })
      }
    }
    if (flow === 'expense') {
      if (step === 'expenseDescription') {
        const question = languageRef.current === 'en-US' ? 'What is the expense amount?'
                      : languageRef.current === 'wo-SN' ? 'Xoolal benn xaalis bi ?'
                      : languageRef.current === 'ar-SA' ? 'ما هو مبلغ المصروف؟'
                      : 'Quel est le montant de la dépense ?'
        return sayAndListen(question, 'expenseAmount', { ...data, description: answer })
      }
      if (step === 'expenseAmount') {
        const question = languageRef.current === 'en-US' ? 'What is the category? Say general if you prefer not to specify.'
                      : languageRef.current === 'wo-SN' ? 'Fan la? Waxal general su fekke.'
                      : languageRef.current === 'ar-SA' ? 'ما هي الفئة؟ قل عام إذا كنت تفضل عدم التحديد.'
                      : 'Quelle est la catégorie ? Dites générale si vous ne souhaitez pas préciser.'
        return sayAndListen(question, 'expenseCategory', { ...data, amount: answer })
      }
      if (step === 'expenseCategory') {
        const confirmMsg = languageRef.current === 'en-US' ? `I will record the expense "${data.description}" of ${spokenMoney(parseNumber(data.amount))}, category ${answer}.`
                          : languageRef.current === 'wo-SN' ? `Dama di bind xaalis "${data.description}" ci ${spokenMoney(parseNumber(data.amount))}, fan ${answer}.`
                          : languageRef.current === 'ar-SA' ? `سأقوم بتسجيل المصروف "${data.description}" بقيمة ${spokenMoney(parseNumber(data.amount))}، فئة ${answer}.`
                          : `Je vais enregistrer la dépense « ${data.description} » de ${spokenMoney(parseNumber(data.amount))}, catégorie ${answer}.`
        return confirmAction(confirmMsg, { ...data, category: /generale|aucune/.test(normalize(answer)) ? 'Générale' : answer })
      }
    }
    if (flow === 'addProduct') {
      if (step === 'productName') {
        const question = languageRef.current === 'en-US' ? 'What is the selling price?'
                      : languageRef.current === 'wo-SN' ? 'Xoolal bi nga jëfandikoo?'
                      : languageRef.current === 'ar-SA' ? 'ما هو سعر البيع؟'
                      : 'Quel est le prix de vente ?'
        return sayAndListen(question, 'productSellingPrice', { ...data, name: answer })
      }
      if (step === 'productSellingPrice') {
        const question = languageRef.current === 'en-US' ? 'What is the buying price?'
                      : languageRef.current === 'wo-SN' ? 'Xoolal bi nga jàpp?'
                      : languageRef.current === 'ar-SA' ? 'ما هو سعر الشراء؟'
                      : 'Quel est le prix d\'achat ?'
        return sayAndListen(question, 'productBuyingPrice', { ...data, sellingPrice: answer })
      }
      if (step === 'productBuyingPrice') {
        const question = languageRef.current === 'en-US' ? 'How many units are currently in stock?'
                      : languageRef.current === 'wo-SN' ? 'Fan yu nekk ci stock bi?'
                      : languageRef.current === 'ar-SA' ? 'كم وحدة موجودة حاليًا في المخزون؟'
                      : 'Combien d\'unités sont actuellement en stock ?'
        return sayAndListen(question, 'productStock', { ...data, buyingPrice: answer })
      }
      if (step === 'productStock') {
        const question = languageRef.current === 'en-US' ? 'What is the minimum stock threshold?'
                      : languageRef.current === 'wo-SN' ? 'Xoolal sàkkub stock bi?'
                      : languageRef.current === 'ar-SA' ? 'ما هو حد المخزون الأدنى؟'
                      : 'Quel est le seuil minimum de stock ?'
        return sayAndListen(question, 'productMinStock', { ...data, stock: answer })
      }
      if (step === 'productMinStock') {
        const confirmMsg = languageRef.current === 'en-US' ? `I will add the product ${data.name}, sold for ${money(parseNumber(data.sellingPrice))}, bought for ${money(parseNumber(data.buyingPrice))}, with ${data.stock} units in stock.`
                          : languageRef.current === 'wo-SN' ? `Dama di togge noppal ${data.name}, di jëfandikoo ${money(parseNumber(data.sellingPrice))}, di jàpp ${money(parseNumber(data.buyingPrice))}, ak ${data.stock} yu nekk ci stock.`
                          : languageRef.current === 'ar-SA' ? `سأقوم بإضافة المنتج ${data.name}، يباع مقابل ${money(parseNumber(data.sellingPrice))}، يشترى مقابل ${money(parseNumber(data.buyingPrice))}، مع ${data.stock} وحدات في المخزون.`
                          : `Je vais ajouter le produit ${data.name}, vendu ${money(parseNumber(data.sellingPrice))}, acheté ${money(parseNumber(data.buyingPrice))}, avec ${data.stock} unités en stock.`
        return confirmAction(confirmMsg, { ...data, minStock: answer })
      }
    }
    if (flow === 'updateProduct') {
      if (step === 'updateProductName') {
        const question = languageRef.current === 'en-US' ? 'What would you like to modify: stock, selling price or buying price?'
                      : languageRef.current === 'wo-SN' ? 'Lu ngay soppi: stock, bi nga jëfandikoo walla bi nga jàpp?'
                      : languageRef.current === 'ar-SA' ? 'ماذا تريد تعديله: المخزون، سعر البيع أو سعر الشراء؟'
                      : 'Que voulez-vous modifier : le stock, le prix de vente ou le prix d\'achat ?'
        return sayAndListen(question, 'updateField', { ...data, productName: answer })
      }
      if (step === 'updateField') {
        const field = /stock|quantite|quantité/.test(normalize(answer)) ? 'stock' : /vente/.test(normalize(answer)) ? 'sellingPrice' : 'buyingPrice'
        const question = languageRef.current === 'en-US' ? (field === 'stock' ? 'What is the new stock quantity?' : 'What is the new amount?')
                      : languageRef.current === 'wo-SN' ? (field === 'stock' ? 'Xoolal yu stock bi ?' : 'Xoolal bi ?')
                      : languageRef.current === 'ar-SA' ? (field === 'stock' ? 'ما هي الكمية الجديدة في المخزون؟' : 'ما هو المبلغ الجديد؟')
                      : (field === 'stock' ? 'Quelle est la nouvelle quantité en stock ?' : 'Quel est le nouveau montant ?')
        return sayAndListen(question, 'updateValue', { ...data, field })
      }
      if (step === 'updateValue') {
        const confirmMsg = languageRef.current === 'en-US' ? `I will modify the ${data.field === 'stock' ? 'stock' : 'price'} of ${data.productName} to ${answer}.`
                          : languageRef.current === 'wo-SN' ? `Dama di soppi ${data.field === 'stock' ? 'stock' : 'bi'} ${data.productName} ci ${answer}.`
                          : languageRef.current === 'ar-SA' ? `سأقوم بتعديل ${data.field === 'stock' ? 'المخزون' : 'السعر'} لـ ${data.productName} إلى ${answer}.`
                          : `Je vais modifier ${data.field === 'stock' ? 'le stock' : 'le prix'} de ${data.productName} à ${answer}.`
        return confirmAction(confirmMsg, { ...data, value: answer })
      }
    }
  }
  ...
  const chooseLanguage = async (code) => { 
    ...
    try {
      ...
      console.warn('⚠️ Permission microphone non accordée ou non disponible', e)
    }
