...
SELECT 
    ...
    p.email,
    p.phone,
    p.name as full_name,  -- Utiliser "name" au lieu de "full_name"
    p.created_at,
    p.updated_at,
...
FROM profiles p
LEFT JOIN auth.users au ON p.id = au.id  -- Jointure correcte avec les UUID
...
-- ÉTAPE 3: Voir les profils avec noms manquants
SELECT '=== PROFILS SANS NOMS ===' as info;
SELECT 
    p.id,
    p.email,
    p.phone,
    p.name,
    au.email as auth_email,
    au.raw_user_meta_data->>'name' as name_disponible,
    au.raw_user_meta_data->>'phone' as phone_disponible
FROM profiles p
...
-- ÉTAPE 4: Mettre à jour tous les noms manquants
UPDATE profiles 
SET 
...
-- ÉTAPE 7: Afficher quelques exemples pour Table Editor
...
-- ÉTAPE 8: Créer le trigger pour les futures inscriptions
...
BEGIN
    ...
    INSERT INTO profiles (
        id,  -- Utiliser l'UUID de l'utilisateur
        email, 
        phone, 
        name,  -- Utiliser "name" au lieu de "full_name"
        created_at,
        updated_at
    ) VALUES (
        NEW.id,
...
SELECT '✅ Vue créée: admin_profiles_view (structure réelle)' as step1;
