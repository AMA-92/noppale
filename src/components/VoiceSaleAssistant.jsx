...
function VoiceSaleAssistant({ products: suppliedProducts = [], sales: suppliedSales = [], expenses: suppliedExpenses = [], formatCurrency, onApply, onConfirm, onProductCreate }) {
  ...
  const start = async () => { speechSessionRef.current += 1; setIsOpen(true); stateRef.current = { flow: 'greeting', step: 'greeting', data: {} }; setPhase('greeting'); setStatus('Je vous écoute… dites bonjour pour commencer.'); setError(''); listenOnce(); try { await refreshData() } catch (e) { console.warn(e) } }
