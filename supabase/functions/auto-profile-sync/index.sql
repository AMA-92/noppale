...
-- S'intègre directement dans la table user_profiles existante

-- ÉTAPE 1: Mettre à jour le trigger existant pour une meilleure synchronisation
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS create_or_update_user_profile();

-- ÉTAPE 2: Créer une fonction améliorée de synchronisation
...
BEGIN
    ...
    -- Insérer ou mettre à jour le profil avec toutes les informations
    ...
    DO UPDATE SET
        ...
        business_name = COALESCE(business_name, user_profiles.business_name),
        updated_at = NOW();
    
    -- Journaliser pour le débogage
    RAISE NOTICE 'Profil synchronisé pour: % | Téléphone: % | Nom: %', 
                 NEW.email, 
...
-- ÉTAPE 4: Créer une fonction de synchronisation manuelle pour les utilisateurs existants
CREATE OR REPLACE FUNCTION sync_existing_user_profiles()
RETURNS TABLE(
    user_id UUID,
    email TEXT,
    phone_synced TEXT,
...
BEGIN
    ...
    -- Traiter aussi les profils existants mais incomplets
    FOR user_record IN 
        SELECT 
            up.user_id,
            up.email,
        ...
        WHERE up.phone IS NULL 
           ...
           OR up.business_name IS NULL
    LOOP
        -- Mettre à jour le profil incomplet
...
-- ÉTAPE 5: Mettre à jour la vue admin_user_profiles pour mieux afficher les informations
...
SELECT 
    ...
    CASE 
        WHEN up.phone IS NOT NULL AND up.full_name IS NOT NULL THEN '✅ Complet'
        WHEN up.phone IS NOT NULL OR up.full_name IS NOT NULL THEN '⚠️ Partiel'
        ELSE '❌ Incomplet'
    END as sync_status,
    
    -- Dernière synchronisation
    CASE 
        WHEN up.updated_at > au.created_at THEN 'Mis à jour'
        ELSE 'Initial'
...
GRANT EXECUTE ON FUNCTION sync_existing_user_profiles TO authenticated;
GRANT SELECT ON admin_user_profiles TO authenticated;

-- ÉTAPE 7: Test de synchronisation pour les utilisateurs existants
SELECT 'Test de synchronisation des profils existants:' as test_info;
SELECT * FROM sync_existing_user_profiles();

-- ÉTAPE 8: Vérification finale
SELECT 'Configuration terminée. Vue mise à jour:' as final_status;
SELECT 
