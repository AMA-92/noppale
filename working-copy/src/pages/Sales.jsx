...
const parseSpokenNumber = (value) => {
  ...
  if (/^\d[\d .]*$/.test(normalized)) return Number(normalized.replace(/[ .]/g, ''))

  let total = 0
  let current = 0
  normalized.replace(/-/g, ' ').split(' ').forEach((token) => {
    if (/^\d+$/.test(token)) current += Number(token)
    else if (frenchNumbers[token] !== undefined) current += frenchNumbers[token]
  ...
  return result || (/(zéro|zero)/i.test(normalized) ? 0 : 1)
}

const escapeRegExp = (value) => String(value).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')

const parseVoiceSale = (transcript, products) => {
  const text = transcript.toLowerCase().replace(/[’']/g, ' ').replace(/\s+/g, ' ').trim()
  const customerMatch = text.match(/(?:client|cliente)\s+(.+?)(?=,|\s+(?:téléphone|telephone|numéro|numero|tel|produit|avec|montant|a donné|a donne|payé|paye)\b|$)/i)
  const phoneMatch = text.match(/(?:téléphone|telephone|numéro|numero|tel)\s*(?:de)?\s*([0-9][0-9 .-]{7,})/i)
  const amountMatch = text.match(/(?:montant reçu|montant recu|a donné|a donne|payé|paye|versement)\s*(?:de|est|:)?\s*([^,.;]+)/i)
  const paymentMethod = /crédit|credit|à crédit|a credit/.test(text)
    ? 'credit'
    : /mobile money|wave|orange money/.test(text)
      ? 'mobile_money'
      : /carte bancaire|par carte/.test(text)
        ? 'carte_bancaire'
        : 'especes'

  const items = (products || []).flatMap((product) => {
    const name = String(product.name || '').trim()
    if (!name || !text.includes(name.toLowerCase())) return []
    const numberPattern = '(\\d[\\d .]*|un|une|deux|trois|quatre|cinq|six|sept|huit|neuf|dix)'
    const before = new RegExp(`${numberPattern}\\s+(?:x\\s*)?${escapeRegExp(name.toLowerCase())}\\b`, 'i').exec(text)
    const after = new RegExp(`${escapeRegExp(name.toLowerCase())}\\s+(?:x\\s*)?${numberPattern}\\b`, 'i').exec(text)
    const quantity = parseSpokenNumber(before?.[1] || after?.[1] || '1')
  ...
  return {
    ...
    customerPhone: phoneMatch?.[1]?.replace(/[ .-]/g, '') || '',
    amountReceived: amountMatch ? parseSpokenNumber(amountMatch[1]) : 0,
    paymentMethod,
    items,
    transcript
  }
...
export default function Sales() {
  ...
  const applyVoiceSale = (transcript, catalog) => {
    const parsed = parseVoiceSale(transcript, catalog)
    return parsed
  }

  const confirmVoiceSale = (parsed) => {
    const total = parsed.items.reduce((sum, item) => sum + item.totalPrice, 0)
    const amountReceived = parsed.amountReceived || 0
    const voiceForm = {
      ...emptySale,
      ...
      notes: `Saisie vocale : ${parsed.transcript}`
    }

    cartItemsRef.current = parsed.items
    setCartItems(parsed.items)
    setForm(voiceForm)
    setShowProductDropdown(false)
    setShowModal(true)
    toast.success('Facture pré-remplie. Vérifiez les détails avant validation.')
  }

  const createVoiceProduct = async (spokenProduct) => {
    const product = {
      name: sanitizeString(truncateString(spokenProduct.name || '', 120)),
      category: '',
      sellingPrice: parseSpokenNumber(spokenProduct.sellingPrice),
      buyingPrice: parseSpokenNumber(spokenProduct.buyingPrice),
      stock: parseSpokenNumber(spokenProduct.stock),
      minStock: parseSpokenNumber(spokenProduct.minStock),
      barcode: '',
      description: 'Ajouté par assistant vocal',
      image: ''
    }
    if (!product.name) throw new Error('Le nom du produit est obligatoire.')
    const saved = await appStorage.addProduct(product)
    await loadProducts()
  ...
  const handleSave = async (e) => {
    ...
    const sanitizedSale = {
      ...
      notes: sanitizeString(truncateString([
        form.customerPhone ? `Téléphone client : ${form.customerPhone}` : '',
        form.notes || ''
          ...
          <div className="sale-modal-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 min-h-0 overflow-hidden">
              <div className="sale-modal-body space-y-4">
                ...
                <section>
                  <label htmlFor="sale-customer-phone" className="label-field">Numéro du client</label>
                  <input
